...
/*** From source: scheduling_system.pml ***/
...
iniglobals(void)
{
		...
		preferencesSent = 0;
		now.anyConflict = 0;
		now.channel = addqueue(1, 1);
#ifdef VAR_RANGES
		logval("response", now.response);
		logval("scheduled", now.scheduled);
		logval("anyConflict", now.anyConflict);
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.channel) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.channel) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.channel) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.channel) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan channel (=%d):	len %d:\t", now.channel, q_len(now.channel));
	c_chandump(now.channel);
	printf("	bit    response:	%d\n", now.response);
	printf("	bit    scheduled:	%d\n", now.scheduled);
	printf("	bit    anyConflict:	%d\n", now.anyConflict);
}
void
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (SchedulingSystem):\n", pid);
		break;
	case 0:
		printf("local vars proc %d (Student):\n", pid);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("conflictNotification"); break;
	case 2: Printf("optimizedSchedule"); break;
	case 3: Printf("requestChange"); break;
	case 4: Printf("submitPreferences"); break;
	default: Printf("%d", x);
	}
}
void
